...
SLURM is the workload manager and job scheduler used for Dilhan.
...
This enables you to effectivly tailor your script to your need when using Dilhan
...
--qos=devel             On Dilhan the *devel* QOS (quality of servive) can be used to submit short jobs for testing and debugging.
==================      ==========================================================================================================
...
With the latter the SLURM manager can distribute your task across all nodes of Dilhan and utilize otherwise unused cores on nodes which for example run a 16 core job on a 20 core node. This usually results in shorter queuing times but slower inter-process connection speeds.
...
of Dilhan i.e. submit the job with ``--ntasks=80`` **if** your application
...
--ntasks-per-node=<num_procs>   Number of (MPI) processes per node. Maximum number depends nodes (16 or 20 on Dilhan)
...
**Remark:** Even if we impose a 28 day run time limit on Dilhan we only give
a weeks warning on system maintenance. Jobs with more than 7 days walltime,
will be terminated and restarted if possible.

See :ref:`about_dilhan` chapter of the documentation if you need more information on the system architecture.
